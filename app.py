...
def build_email_body(supplier_name, inv_shapes, miss_col, url_counts, cut_miss, pdf_miss, price_miss):
    ...
    if inv_shapes or miss_col.get("shape"):
        lines.append("1. Shape")
        if miss_col.get("shape"): lines.append(f"- Shape is missing for {miss_col['shape']} item(s).")
        if inv_shapes:
